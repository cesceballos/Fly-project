...
%Isolate step dependent parameters (4 per fly)
Comb_mat_2 = Comb_mat_test(:,1:5);
for i = 1:6 %6 legs
    Comb_mat_2 = [Comb_mat_2 Comb_mat_test(:,(32*(i-1)+10):(32*(i-1)+37))];
end
Comb_mat_test = Comb_mat_2;
...
rms = sqrt(sum((ball_vel_2(:,i)-abs(predicted_output)).^2)/length(predicted_output));
all_rms = [all_rms rms];
...
for k = 1:num_flies
    ...
    hold on
    plot(t,predicted_output((510*(k-1)+1):(510*k)),'g')
    plot(t,abs(predicted_output((510*(k-1)+1):(510*k))),'m')
    ylabel('Z-score')
    ylabel('Velocity')
    xlabel('s')
    xlim([0 2.55])
    ...
    if k ==1
        ...
        legend({'Real' 'Prediction' 'Flipped pred'})
        legend box off
    end
end
sgtitle('P9LT for training. P9RT for testing')
